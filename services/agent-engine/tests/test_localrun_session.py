...
class TestPreviewPortAllocation(unittest.TestCase):
    ...
    def test_start_preview_app_allocates_and_threads_ports_into_plan(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch(
                    "omnistackai_agent_engine.localrun.run.allocate_free_ports",
            ...
            # R-542/R-545/R-553: ports are allocated for the admin console, the Expo dev server
            # and any further surface apps/ holds. This repo has none of them, so the plan stays
            # single-app and asks for exactly four.
