...
> **The differentiating SPINE has begun.** R-430 ships the **Ecosystem Scope Compiler**: a business prompt → a multi-app ecosystem proposal (customer app + merchant/driver/admin portals) with Complete / Customer-only / Custom build-scope options + ≤3 materiality questions — the "wow" competitors lack. Deterministic, offline, testable. See it: `task agent-engine:scope:propose -- "Create a food delivery app …"`. Foundation before this: front door **R-416**–**R-426** (chat → create → run → preview) · **R-427/R-428** generated apps compile & render · **R-429** generated apps pass strict `tsc --noEmit` (0 errors; `task agent-engine:web-typecheck` PASSED for both examples). UI-component series PAUSED at R-415 (resumable). **NEXT (founder-approved direction):** R-431 map each proposed AppSurface → an Application IR so "Complete Business Platform" materializes MULTIPLE owned repos from one prompt; and/or an opt-in cheap-LLM refinement layer over the deterministic classifier; optionally wire a frontier model for generation quality.

## Repo/workflow state

- **R-430 (Ecosystem Scope Compiler — first spine brick)** shipped: new stdlib-only `intake/scope_compiler.py` turns a prompt into a framework-neutral `ScopeProposal` (domain + confidence + matched keywords, actors, multi-surface ecosystem, Complete/Customer-only/Custom options, ≤3 questions) via a deterministic 10-domain `DOMAIN_LIBRARY` + weighted keyword `classify_domain()` + `propose_ecosystem()` (with a `custom-application` fallback). Pure/deterministic (0 model/network), exported from `intake/__init__.py`. Deterministic CLI `intake/scope_propose.py` → `task agent-engine:scope:propose -- "<prompt>"`. `tests/test_scope_compiler.py` (11 tests). `task verify` **2,888** pass; lint/security/env + demos (152/149) green; 0 model calls. NOT yet wired into IR/repo generation (next brick). Single commit authored `sanjeetji <sk698166@gmail.com>`.
