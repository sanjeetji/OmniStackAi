...
| R-563 ✅ | **Make an ecosystem project editable.** *(Rescoped 2026-09-25 — the original premise was wrong.)* The console is already on the disk-backed workspace store: its edit path is `/projects/{id}/edit` → `/api/workspaces/{id}/edit`, the IR is written to `ir.json` on every build, and a fresh store instance loads it back. Verified. The in-memory LRU serves only the standalone Studio's build-id path, which the console labels legacy. **The real gap:** `live_serve.py:159` leaves ecosystem builds with `editable_ir = None`, so no IR is saved and a four-app project answers an edit with "has no IR; build first" — on a project the user just built. `plan_edit` also assembles with `assemble_project`, which would delete every surface app if pointed at an ecosystem repo. | L | The flagship capability is the one that cannot be changed afterwards |
