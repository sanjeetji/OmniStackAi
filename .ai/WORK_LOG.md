...
- **Screenshots, R-528's blocker solved.** `scripts/capture-template-screens.mjs` (generic, reusable for the remaining nine templates) drives Playwright from a scratch folder, never a platform or template dependency. Each template describes its own capture in `capture.mjs`: sign in as the demo users, stage live scenes through the API, shoot every page. For RideNow that is 48 screens plus a composed cover, including the real incoming-offer sheet, the live trip and the live ops map.
- **Published.** `templates/catalog/ride-now` v1.0.0 with four apps, a cover and eight highlight screenshots; the catalogue lists it and *Use template* works end to end.
- **Five platform defects found by driving a preview with a real browser for the first time** (every earlier check was curl, which hides all of them): the preview proxy forwarded the upstream `content-encoding` although the body had been decoded, so no preview page loaded in any browser; previews ran dev servers whose hot-reload socket cannot pass the proxy, and a Turbopack dev app never hydrates without it, so nothing was interactive — previews now build once and serve the production build (RideNow ready in 46 s); `.DS_Store` was copied into every project made from a template and counted in its digest; a project's Python tests reported 0 passed wherever pytest is missing (Python 3.11 changed `unittest discover`); git's background `gc` wrote inside `.git` while the platform copied or removed a project.
- **Recorded, not fixed:** client-side navigation in previews falls back to full page loads, because the console is itself a Next app and answers RSC navigation requests to `/preview/...` before the proxy code runs. Needs a preview path that is not a Next route.
- **Gates:** `task verify` 3,980 OK (0 model calls), `task lint`, `task security:quick`, `task env:check`, `scripts/test.sh` (new R-530 block), `go test ./...` 22 packages, console typecheck/lint/build. Live: 42/42 page checks through the authenticated proxy, `workflow.test.ts` 5/5 against the preview, role isolation 403s, and a browser check of the ops console rendering live data through the proxy.
- **Outstanding:** publish checklist item 9 (three scripted chat edits) and `smoke-core.sh` need a model provider; this machine has neither a key nor Ollama.
