...
Version: 6.0  
Date: 2026-09-05  
Purpose: Machine-friendly implementation rules for developers and AI coding agents. V5 turns the R&D blueprint into a cost-aware, production-oriented multi-platform software factory: instant-browser-first verification, QR/real-device delivery, durable project memory separated from chat, progressive context retrieval, LLM escalation/cost control, Temporal durability, database recommendation profiles, client handover, release provenance and current competitor benchmarks.
...
# V6 Execution Addendum — AI/Developer Interoperability, Local Models and Resumable Delivery

Version: 6.0  
Date: 2026-09-05  
Status: **Normative. These V6 rules override older sections where they conflict.**

## 80. V6 R&D Review Verdict

The V5 foundation is strong and production-oriented: Application IR, change-impact control, durable execution, progressive context, provider interfaces, isolated runtimes, Git/PR recovery, production gates, supply-chain controls and cost-per-successful-task are the correct core ideas.

V6 closes the main execution gaps that would otherwise make the blueprint difficult to hand directly to Codex, Claude Code, Cursor, a local coding model, or a new human developer:

- explicit tool-neutral repository instructions and document precedence,
- a first-class **Local Model Provider** for Ollama-compatible models such as Qwen plus cloud API providers,
- model capability/context-window registry and token/context budgeting,
- deterministic project state, current-task and handoff files,
- start/resume/stop protocols that do not depend on chat history,
- environment/secrets contracts,
- stronger production migration/release/recovery controls,
- explicit agentic threat models and local-model evaluation gates,
- a Phase 0 foundation before feature implementation.

The design goal is not to copy Emergent or any single competitor. The goal is **smooth UX + verifiable engineering + portable source + lower verified-change cost**.

## 81. Canonical Document and Repository Precedence

Every AI tool and developer must resolve conflicting instructions in this order:

1. security policy and human-approved production policy,
2. current Git source + lockfiles + executable contracts,
3. `.ai/PROJECT_STATE.yaml` and `.ai/CURRENT_TASK.yaml`,
4. Application IR + accepted requirements,
5. ADRs + architecture policies,
6. API/DB schemas and migrations,
7. tests/baselines and current runtime evidence,
8. `AGENTS.md` / `CLAUDE.md` / `.cursor/rules/*`,
9. `docs/START_HERE.md` and runbooks,
10. accepted handoff/task history,
11. raw chat/history.

**Never let an old chat message silently override current Git, IR, ADRs, tests or project state.**

Required generated-project root additions:

```text
<org>-<product>/
  AGENTS.md
  CLAUDE.md
  .cursor/rules/omnistackai.mdc
  .ai/
    PROJECT_STATE.yaml
    CURRENT_TASK.yaml
    HANDOFF.md
...
`AGENTS.md` is the portable cross-agent root instruction. Tool-specific files may add ergonomics, but **must not contradict the portable contract**.

## 82. AI and Human Start Protocol

Before changing code, any AI coding tool or developer MUST:

```text
1. Read AGENTS.md.
2. Read docs/START_HERE.md.
3. Read .ai/PROJECT_STATE.yaml.
4. Read .ai/CURRENT_TASK.yaml.
5. Read only the relevant IR/ADR/policy files.
6. Check git status, current branch, HEAD and uncommitted diff.
7. Run `task doctor`.
8. Run `task bootstrap` only if dependencies are missing.
9. Run the smallest baseline verification for affected modules.
10. Restate: current phase, task ID, objective, expected blast radius, blockers and next action.
11. Only then edit code.
```

If `CURRENT_TASK.yaml` is empty, select the next eligible task from the execution tracker only after dependencies and phase gates are satisfied. Do not skip ahead just because a later task is easier.

## 83. Pause, Stop, Handoff and Resume Protocol

A work session can stop at any time without losing engineering state.

### Before stopping

The agent/developer must update:

- `.ai/CURRENT_TASK.yaml` — exact state and next deterministic action,
- `.ai/WORK_LOG.md` — concise factual work performed,
- `.ai/HANDOFF.md` — branch, commit, changed files, test evidence, blockers, risks, next command,
- tracker task status/evidence,
- Application IR/ADR only when architecture/product truth changed.

Prefer a stable checkpoint commit when the tree is valid. Never make a fake “done” commit to hide failing work.

### Resume algorithm

```text
Read state -> verify branch/HEAD -> inspect working tree -> compare handoff commit
-> run task doctor -> re-run last relevant proof -> continue NEXT_ACTION
```

If recorded state disagrees with Git, **Git wins** and the agent must repair the state files before implementation.

Minimum handoff payload:

```yaml
task_id: OSAI-MVP-001
status: in_progress
phase: MVP
branch: ai/OSAI-MVP-001-provider-registry
head_sha: <git-sha>
last_verified_sha: <git-sha>
completed:
  - ModelProvider interface scaffolded
verification:
  - command: task test:model-gateway
    result: pass
blockers: []
next_action: Implement OllamaOpenAICompatibleAdapter health/model discovery.
next_command: task test:model-gateway
allowed_paths:
  - services/agent-orchestrator-python/**
  - packages/contracts/**
```

## 84. Model Fabric — Cloud API + Local Ollama/Qwen

All model access goes through `ModelProvider` / `ModelGateway`. Product/domain code must not know whether inference is OpenAI, Anthropic, another API provider, Ollama on localhost, or an enterprise private endpoint.

```text
Agent/Service
   -> ModelGateway
      -> Router + Policy + Budget + Eval profile
         -> OpenAIAdapter
         -> AnthropicAdapter
         -> Gemini/other adapter
         -> OllamaAdapter
         -> OpenAICompatiblePrivateAdapter
```
...
- configurable base URL, default local-only binding,
- model discovery and health check,
- chat/generation,
- streaming,
- tool/function calling **only when the selected model proves capability**,
- JSON/structured output where supported,
- embeddings through an embedding-capable local model,
- per-model context-window configuration,
- max-output limit,
- timeout/cancellation,
- concurrency limit and back-pressure,
- optional GPU/CPU/RAM telemetry,
- no assumption that “local = free” — track latency, energy/hardware occupancy and developer wait time.

Qwen is a **model family option**, not a hard-coded dependency. The registry may include Qwen, Llama, Gemma, DeepSeek-derived or future models if license, capability and evaluation gates pass.
...
```text
Deterministic tool
-> local small/cheap model when quality threshold is met
-> low-cost cloud model
-> strong cloud/private reasoning model for hard/high-risk tasks
-> independent review model for selected critical changes
```

Privacy policy can force `local_only`, `private_only`, approved-provider allowlists, or redacted cloud context.

## 85. Model Capability Registry and Context/Token Contract

Never route only by model name. Persist capabilities per exact provider/model/version/configuration:

```yaml
provider_id: ollama-local
model_id: qwen3-coder:<pinned-tag>
endpoint_class: local
capabilities:
  text: true
  streaming: true
  tools: evaluated
  structured_output: evaluated
  vision: false
  embeddings: false
context:
  advertised_tokens: <configured>
  safe_input_budget: <measured>
  reserved_output_tokens: <measured>
  reserve_for_tools_and_system: <measured>
quality:
  coding_eval_version: v1
  tool_call_success_rate: <metric>
  patch_acceptance_rate: <metric>
operations:
  p50_latency_ms: <metric>
  p95_latency_ms: <metric>
  max_concurrency: <metric>
policy:
...
- reserved output
- safety margin
= maximum task Context Pack
```

The Context Manager must estimate tokens using the target provider/model tokenizer when available, otherwise a conservative estimator. On overflow: shrink low-authority context first, summarize logs deterministically, retrieve symbols on demand, or route to an approved larger-context model. **Never silently truncate acceptance criteria, security policy, current task state or relevant contracts.**

Track input, cached input, output, reasoning/hidden-billed tokens when provider exposes them, request count, latency and monetary cost. Do not infer hidden chain-of-thought content.

## 86. AI Tool Interoperability Contract

### Codex and portable agents

`AGENTS.md` contains the project-wide executable working agreement: source-of-truth order, allowed commands, branch/task policy, testing, security, stop/resume protocol and protected paths.

### Claude Code

`CLAUDE.md` should be a thin compatibility layer that says to follow `AGENTS.md`, then lists only Claude-specific safe workflow notes. Avoid maintaining a second architecture manual.

### Cursor

`.cursor/rules/omnistackai.mdc` should point to the same portable rules and may scope extra UI/editor behavior. `.cursorignore` (or equivalent current ignore mechanism) must exclude secrets, credentials, private keys and local sensitive artifacts from model context.

### Universal rule

Tool-native “rules”, memories, hooks and commands improve ergonomics; **CI, policy code, Git protections and tests enforce guarantees**. Never rely on an LLM instruction file as the only security control.

## 87. Environment and Secret Contract
...
- startup validation and fail-fast missing-config errors,
- environment-specific secret references, never copied values,
- separate data stores/credentials between test/staging/prod,
- seeded local/test data that contains no production personal data,
- documented external service sandbox/test modes,
- environment parity checks for runtime/toolchain versions.

AI context exclusions include `.env*` except `.env.example`, credentials, private keys, signing material, production exports, database dumps and sensitive diagnostics unless a policy-approved redaction pipeline is used.

## 88. Testing and Quality Expansion

Add to existing gates:

- property-based tests for critical pure logic,
- migration **expand/contract** compatibility tests,
- rollback/roll-forward rehearsal for stateful releases,
- contract compatibility between old/new clients and servers,
- concurrency/race/idempotency tests for agent/task/billing systems,
- prompt-injection/tool-output poisoning test corpus,
- model/provider conformance tests,
- local-model quality/tool-call/structured-output evals before routing production tasks,
- deterministic replay tests for checkpoints/event ledger,
- accessibility and visual regression for user-facing surfaces,
- performance budgets by target,
- chaos/recovery tests for queues, sandbox provider, model provider and storage failure.

A local model is never promoted because it is cheap; it is promoted because it passes the task-class acceptance threshold.

## 89. Production, Deployment and Operations Expansion

Production release requires immutable source/artifact provenance plus:

```text
build once -> promote same digest -> canary/blue-green -> observe SLOs
-> expand traffic -> automatic/manual rollback trigger -> retained known-good artifact
```

For database changes use expand/migrate/contract, backward-compatible APIs during rollout, resumable backfills, explicit rollback/roll-forward decisions, migration locks and verified backups.
...
- backup/restore and disaster-recovery drills,
- status/incident/postmortem workflow,
- dependency/security patch SLA,
- provider outage/failover runbooks,
- cost anomaly kill switches,
- audit-retention and privacy deletion/export controls.

## 90. Security and Agentic Threat Model Expansion

Threat-model at minimum:

- repository prompt injection,
- malicious issue/doc/tool output,
- MCP/tool schema poisoning,
- secret exfiltration through prompts/logs/diffs,
- sandbox escape and cross-tenant access,
- dependency/package substitution,
- untrusted generated scripts,
- source-control token abuse,
- malicious or compromised model/provider endpoint,
- local endpoint exposed beyond loopback without authentication,
- production action replay after resume,
- approval confusion/substitution,
- data leakage through embeddings/vector stores/caches.

Controls: default-deny capabilities, path/network scopes, content provenance, secret broker, short-lived credentials, egress allowlists, isolated runners, idempotency keys, immutable audit events, signed artifacts, dependency pinning, SBOM/provenance, protected production approvals and periodic red-team/eval suites.

## 91. Phase 0 to Production Execution Sequence

Do not ask an AI tool to “build the whole platform” in one prompt.

### PHASE 0 — Foundation / Repo Bootstrap

1. monorepo root, toolchains and lockfiles,
2. `AGENTS.md`, `START_HERE`, state/handoff files,
3. Taskfile commands: `doctor`, `bootstrap`, `lint`, `test`, `verify`, `ai:status`, `ai:handoff`,
4. typed config + `.env.example` + secret exclusions,
5. CI skeleton + protected main + CODEOWNERS,
6. baseline telemetry/error format,
7. ADR-0001 monorepo and provider-boundary decisions.

Exit gate: a new developer or fresh AI session can clone, run `task doctor && task bootstrap && task verify`, understand the next task and stop/resume without chat history.

### MVP

Implement the existing V5 MVP roadmap, but add the ModelProvider registry and **Ollama adapter as an early supported provider**, with cloud provider adapters behind the same contract. Cloud API keys are optional configuration, not architecture dependencies.

### MID

Add richer local desktop execution, BYOK, model/provider benchmarks, remote device/debug workflows, advanced environment parity, visual/accessibility gates and store automation.

### ADVANCED

Add private/BYOC runners, private model endpoints, hardened microVMs when economics justify, policy-as-code, multi-region, model benchmark automation and enterprise multi-repo only from measured need.

### PRODUCTION

No production label until the blocking production gates have evidence. Add explicit evidence for local-model endpoint security, provider conformance/failover, agent handoff/recovery and database progressive migration.

## 92. V6 Non-Negotiable Rules

1. One portable repository contract; tool-specific instructions are adapters, not the source of truth.
2. A new AI session must be able to start from Git + state files without old chat.
3. Every stopped session leaves a deterministic handoff with exact next action and verification state.
4. Local Ollama-compatible and cloud API providers use the same `ModelProvider` boundary.
5. No model is trusted by brand/name alone; capabilities and quality are measured per version/configuration.
6. Context is budgeted per target model; critical instructions/contracts are never silently truncated.
7. Secrets are capabilities/references, never normal AI context.
8. Local inference is private/cost-efficient only when endpoint exposure, model license, quality and hardware cost are controlled.
9. `task doctor`, `task verify`, CI and policy gates are the executable truth of onboarding quality.
10. AI instructions guide; deterministic hooks/tests/policies enforce.
11. Production deploys promote immutable artifacts and use progressive rollout + proven rollback/roll-forward.
12. Database changes are backward-compatible and migration-safe by default.
