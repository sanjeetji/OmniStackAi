...
import packs_data as packs  # noqa: E402
...
OUT_PACKS = FOLDER / "PACK_CATALOG.md"
...
def build() -> tuple[list[Task], list[Task]]:
    ...
    for pk in pack_rows():
        tid = f"{'HP' if pk['layer'] == 'Horizontal' else 'VP'}-{pk['id']}"
        status = "Completed" if tid in changelog else pk["status"]
        rows[tid] = Task(
            id=tid, title=f"{pk['layer']} pack {pk['id']}: {pk['provides']}", source=pk["source"],
            area=pk["group"], phase=phase_for(status, pk["tracked_in"]), status=status,
            priority=pk["priority"], tracked_in=pk["tracked_in"], notes=pk["note"],
        )
...
H_WAVE = {"P0": "PC-073", "P1": "PC-073", "P2": "PC-078", "P3": "PC-079"}
V_WAVE = {1: "PC-074", 2: "PC-080", 3: "PC-081"}


def pack_rows() -> list[dict]:
    """The pack catalog as rows, checked: ids unique, aliases resolve, every industry has surfaces."""
    out: list[dict] = []
    for group, items in packs.HORIZONTAL:
        for pid, provides, source, prio, status, note in items:
            out.append(dict(layer="Horizontal", group=group, id=pid, provides=provides, source=source,
                            priority=prio, status=status, note=note, tracked_in=H_WAVE[prio],
                            surfaces="API + Web + App + Admin, each stack (RN first; Kotlin/Swift in PC-082)"))
    for domain, bundle, wave, _src, items in packs.VERTICAL:
        if bundle not in packs.DOMAIN_SURFACES:
            raise SystemExit(f"industry {domain} ({bundle}) has no surfaces in DOMAIN_SURFACES")
        surfaces = "; ".join(f"{r}: {s}" for r, s in packs.DOMAIN_SURFACES[bundle])
        for pid, provides, source, status, note in items:
            out.append(dict(layer="Vertical", group=f"{domain} ({bundle})", id=pid, provides=provides,
                            source=source, priority=packs.WAVE_PRIORITY[wave], status=status, note=note,
                            tracked_in=V_WAVE[wave], surfaces=surfaces))
    for layer in ("Horizontal", "Vertical"):
        ids = [r["id"] for r in out if r["layer"] == layer]
        dup = {i for i in ids if ids.count(i) > 1}
        if dup:
            raise SystemExit(f"duplicate {layer.lower()} packs: {sorted(dup)}")
    known = {r["id"] for r in out}
    bad = {k: v for k, v in packs.ALIASES.items() if not v.startswith("(") and v not in known}
    if bad:
        raise SystemExit(f"aliases point at packs that do not exist: {bad}")
    return out


def write_pack_catalog() -> None:
    rows = pack_rows()
    h = [r for r in rows if r["layer"] == "Horizontal"]
    v = [r for r in rows if r["layer"] == "Vertical"]
    lines = [
        "# Pack catalog", "",
        "> Generated by `tools/build_master_tasks.py` from `tools/packs_data.py`. Reconciles "
        "`R_&_D/specs/pack-framework/03-04`, `docs/PACK_REGISTRY.md`, the PRD's blueprints and "
        "stack-aware packs, and packs added on 2026-09-26. Every pack is also a row (`HP-*`, "
        "`VP-*`) in the master list.", "",
        f"**{len(h)} horizontal packs in {len(packs.HORIZONTAL)} categories; {len(v)} vertical packs "
        f"in {len(packs.VERTICAL)} industries.**", "",
        "Stack-aware: every pack has an implementation per stack — Next.js (web, admin), React Native "
        "(app), Kotlin/Compose and Swift/SwiftUI (native app, Phase 5), Python/Go/Node (API).", "",
        f"**Surfaces rule for vertical packs:** {packs.SURFACE_RULE}", "",
    ...
    for group, _items in packs.HORIZONTAL:
        mine = [r for r in h if r["group"] == group]
        lines += [f"### {group} ({len(mine)})", "",
                  "| Pack | Provides | Status | Priority | Source | Notes |", "|---|---|---|---|---|---|",
                  *[f"| `{r['id']}` | {md_cell(r['provides'])} | {r['status']} | {r['priority']} | "
                    f"{md_cell(r['source'])} | {md_cell(r['note'])} |" for r in mine], ""]
    lines += ["## Vertical packs, by industry", ""]
    for domain, bundle, wave, src, _items in packs.VERTICAL:
        mine = [r for r in v if r["group"] == f"{domain} ({bundle})"]
        lines += [f"### {domain}: `{bundle}` (wave {wave}, {len(mine)} packs)", "",
                  f"Source: {src}. Surfaces: " + "; ".join(
                      f"**{r}** {s}" for r, s in packs.DOMAIN_SURFACES[bundle]) + ".", "",
                  "| Pack | Provides | Status | Source | Notes |", "|---|---|---|---|---|",
                  *[f"| `{r['id']}` | {md_cell(r['provides'])} | {r['status']} | {md_cell(r['source'])} | "
                    f"{md_cell(r['note'])} |" for r in mine], ""]
    lines += ["## Renamed packs", "", "| Old name | Name here |", "|---|---|",
              *[f"| {k} | {v} |" for k, v in packs.RENAMES.items()], ""]
    lines += ["## Names docs/PACK_REGISTRY.md depends on but never defined", "",
              "The composer would fail on every one of these. Each now resolves to a pack here.", "",
              "| Dependency name | Resolves to |", "|---|---|",
              *[f"| `{k}` | {v if v.startswith('(') else f'`{v}`'} |" for k, v in sorted(packs.ALIASES.items())], ""]
    OUT_PACKS.write_text("\n".join(lines), encoding="utf-8")
...
def write_xlsx(queue: list[Task], everything: list[Task]) -> bool:
    ...
    table(wb.create_sheet("Pack Catalog"),
          ["Layer", "Category / industry", "Pack", "Provides", "Surfaces / roles", "Status", "Priority",
           "Wave task", "Source", "Notes"],
          [[r["layer"], r["group"], r["id"], r["provides"], r["surfaces"], r["status"], r["priority"],
            r["tracked_in"], r["source"], r["note"]] for r in pack_rows()],
          [11, 30, 22, 50, 50, 14, 9, 10, 26, 40], status_col=5, prio_col=6)
