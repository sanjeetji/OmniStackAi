...
export function TrendChart({
  ...
  if (series.length === 0) {
    return <p className="px-3 py-8 text-center text-[12px] text-[var(--muted-light)]">Nothing to chart yet.</p>;
  }
        ...
        return (
          <li key={row.label}>
            <div className="flex items-baseline justify-between gap-2 text-[12.5px]">
              ...
              <span className="tabular shrink-0 text-[var(--muted-light)]">
                {format(row.value)} · {share}%
              </span>
            </div>
            <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-slate-800">
              <div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${Math.max(share, 2)}%` }} />
            </div>
            {row.note && <p className="mt-0.5 text-[11px] text-[var(--muted)]">{row.note}</p>}
          </li>
        );
      })}
    </ul>
