...
export class SchedulingService {
  ...
  static async bookAppointment(input: BookAppointmentInput) {
    return withTransaction(async (client) => {
      ...
      // 2. Generate sequential appointment number
      const countRes = await client.query(`SELECT COUNT(*) as total FROM appointments`);
      const seq = parseInt(countRes.rows[0].total, 10) + 1;
      const aptNumber = `APT-2026-${seq.toString().padStart(5, "0")}`;
      ...
      const invCountRes = await client.query(`SELECT COUNT(*) as total FROM invoices`);
      const invSeq = parseInt(invCountRes.rows[0].total, 10) + 1;
      const invNumber = `INV-2026-${invSeq.toString().padStart(5, "0")}`;
  ...
  static async cancelAppointment(
    ...
    return withTransaction(async (client) => {
      ...
      if (apt.is_paid && apt.invoice_id && refundPolicy.refundAmount > 0) {
        const refCountRes = await client.query(`SELECT COUNT(*) as total FROM refunds`);
        const refSeq = parseInt(refCountRes.rows[0].total, 10) + 1;
        const refNumber = `REF-2026-${refSeq.toString().padStart(5, "0")}`;
