...
- **Diff Predictability & Safety**:
  - 100% diff-invariant across `ir.description` changes; zero external npm packages.
  - Registered in `NextjsWebAdapter.generate()` as a `GeneratedFile` at `components/context-menu.tsx` and exported in `codegen.__init__` as `render_context_menu_component`.
