...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 1,459 tests pass)
...
WHAT TO DO NEXT (pick with the founder; all continue the builder), continue from R-343
...
(R-343) with its task contract before writing code.
```
