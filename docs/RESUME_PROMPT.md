...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 2,934 tests pass)
...
Started; MVP 147/253 = 58.1%). Work past R-358 now covers 77 completed tasks: 57 reusable UI-component
...
type cleanup (R-429), and the first six differentiating-spine bricks (R-430 -> R-435).
...
R-435; `task verify` = 2,934 tests. R-416 added prompt-to-IR intake, R-417 materialized a generated
...
WHAT TO DO NEXT
- R-430 through R-435 are DONE — the DIFFERENTIATING SPINE now proposes a curated multi-app ecosystem,
  ...
  in ecosystem planning without changing generated output.
- NEXT R-436 (write `.ai/CURRENT_TASK.yaml` + `.ai/tasks/R-436.md` before coding): define an immutable,
  declarative Solution Pack customization/delta manifest pinned to pack id/version/digest, with bounded typed
  changes and deterministic validation/JSON. Do not apply mutations or call a model yet.
- After R-436: apply validated deterministic configuration, then add bounded AI-delta generation;
...
Begin by reading the files above and running the start protocol, then continue the spine at R-436
(declarative Solution Pack customization/delta manifest, per WHAT TO DO NEXT) and write its Standard AI Task Contract
before writing code.
