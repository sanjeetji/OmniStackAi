...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 2,945 tests pass)
...
Started; MVP 147/253 = 58.1%). Work past R-358 now covers 78 completed tasks: 57 reusable UI-component
...
type cleanup (R-429), and the first seven differentiating-spine bricks (R-430 -> R-436).
...
R-436; `task verify` = 2,945 tests. R-416 added prompt-to-IR intake, R-417 materialized a generated
...
WHAT TO DO NEXT
- R-430 through R-436 are DONE — the DIFFERENTIATING SPINE now proposes a curated multi-app ecosystem,
...
- NEXT R-437 (write `.ai/CURRENT_TASK.yaml` + `.ai/tasks/R-437.md` before coding): deterministically apply
  only validated `configuration` intents to a fresh Application IR loaded from the exact pinned pack. Return
  a validate_ir-clean derived IR plus transparent applied/unapplied provenance; every `ai-delta` intent stays
  explicitly unapplied. Do not generate source, build, or call a model yet.
- After R-437: add bounded AI-delta generation through the provider boundary;
...
Begin by reading the files above and running the start protocol, then continue the spine at R-437
(deterministic configuration application to a fresh pinned pack IR, per WHAT TO DO NEXT) and write its Standard AI Task Contract
before writing code.
