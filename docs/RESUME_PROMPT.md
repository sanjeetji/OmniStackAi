...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 2,888 tests pass)
...
Started; MVP 147/253 = 58.1%). Work past R-358 now covers 72 completed tasks: 57 reusable UI-component
...
inline-style fix (R-427), and generated-app compile fixes + an opt-in tsc gate (R-428).
...
R-430; `task verify` = 2,888 tests. R-416 added prompt-to-IR intake, R-417 materialized a generated
...
platform. NOT yet wired into IR/repo generation (next brick). No tracker workbook row exists past R-358.
...
WHAT TO DO NEXT
- R-430 is DONE — the DIFFERENTIATING SPINE has begun (founder-approved): the Ecosystem Scope Compiler turns
  a business prompt into a deterministic multi-app ecosystem proposal
  (`task agent-engine:scope:propose -- "<prompt>"`; intake/scope_compiler.py + test_scope_compiler.py).
- CONTINUE THE SPINE (founder-approved direction; write `.ai/CURRENT_TASK.yaml` + `.ai/tasks/R-###.md` before
  coding). Highest-leverage next bricks:
  (a) **R-431: map each proposed `AppSurface` -> an Application IR** so choosing "Complete Business Platform"
  materializes MULTIPLE owned repos/apps from one prompt (build on the existing assembler + git-service that
  already turn ONE IR into an owned repo); and
  (b) **an opt-in cheap-LLM refinement layer** over the deterministic classifier (mirror nl_to_ir.py core vs
  live_run.py) for prompts outside the curated domains.
  Solution Packs (pre-tested monorepo skeletons + AI-delta generation) and wiring a frontier model for
  generation quality remain on the spine roadmap. Keep `task verify` model/Docker/DB/install/network-free
  (any live/model path stays opt-in); preserve single-session ownership and explicit trusted-local mode.
...
Begin by reading the files above and running the start protocol, then continue the spine (R-431: scope
surfaces -> Application IRs, per WHAT TO DO NEXT) and write its Standard AI Task Contract before writing code.
