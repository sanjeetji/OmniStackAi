...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 1,535 tests pass)
...
WHAT TO DO NEXT (pick with the founder; all continue the builder), continue from R-348
...
(R-348) with its task contract before writing code.
