...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 2,897 tests pass)
...
Started; MVP 147/253 = 58.1%). Work past R-358 now covers 73 completed tasks: 57 reusable UI-component
...
R-431; `task verify` = 2,897 tests. R-416 added prompt-to-IR intake, R-417 materialized a generated
...
WHAT TO DO NEXT
- R-430 + R-431 are DONE — the DIFFERENTIATING SPINE's first two bricks (founder-approved): the Ecosystem
  Scope Compiler *proposes* a multi-app ecosystem (`task agent-engine:scope:propose`), and Scope -> Application
  IRs *materializes* it — one prompt -> MULTIPLE owned, clean-compiling app repos
  (`task agent-engine:ecosystem:plan` / `:build`; intake/ecosystem.py + test_ecosystem.py).
- CONTINUE THE SPINE (founder-approved direction; write `.ai/CURRENT_TASK.yaml` + `.ai/tasks/R-###.md` before
  ...
  (a) **an opt-in cheap-LLM refinement layer** over the deterministic domain classifier + curated
  DOMAIN_ENTITIES data models (mirror nl_to_ir.py core vs live_run.py) so prompts OUTSIDE the 10 curated
  domains still get a tailored ecosystem;
  (b) **surface-specific entity focus/roles** so each generated app is more tailored than sharing the full
  domain model across surfaces; and
  (c) **Solution Packs** (pre-tested monorepo skeletons + AI-delta generation) plus optionally a frontier
...
Begin by reading the files above and running the start protocol, then continue the spine (next: an opt-in
cheap-LLM refinement layer over the deterministic classifier, per WHAT TO DO NEXT) and write its Standard AI
