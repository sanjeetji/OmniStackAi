...
WHAT IS ALREADY BUILT (platform generators are Python 3.13 stdlib-only, offline, in services/agent-engine; 858 tests pass)
...
- BUILDER (the product): Application IR (framework-neutral spec) + semantic validator/canonical
  ...
  deletes are optimistic too (R-291). Data-loading states render layout-preserving skeleton placeholders
  instead of plain "Loading..." text (R-292: collection table, subcollection lists, detail main).
...
backfilled); keep it current going forward. It now has 292 unique rows: 81 Done, 1 Deferred, 210 Not
Started; MVP is 81/187 (43.3%). The earlier reported R-251 MVP baseline of 145 was one low—direct recount
is 146, and R-252..R-292 added 41 rows. The summary above is current through R-292; Git, state files,
...
WHAT TO DO NEXT (pick with the founder; all continue the builder), continue from R-293
- Offline-doable now: loading skeletons now cover the collection table, subcollection lists, and detail
  main (R-292), so R-293 could extend them to the two remaining "Loading..." spots (form edit-mode
  initial load, detail record-selector list), or add another generated-app UX/robustness increment. Reuse
  the proven patterns, preserve public hook signatures, and add focused generation tests first.
...
(R-293) with its task contract before writing code. Commit to main.
```
