...
WHAT IS ALREADY BUILT (all Python 3.13 stdlib-only, offline, in services/agent-engine; 256 tests pass)
...
- BUILDER (the product): Application IR (framework-neutral spec) + semantic validator/canonical
  ...
  routes stay 501), when the IR has entities + database_strategy=postgres. TRI-TARGET proven from ONE
  IR; all offline/deterministic (emit
...
(handlers call the repositories). Do NOT overwrite backlog rows; continue from R-241.
...
WHAT TO DO NEXT (pick with the founder; all continue the builder), continue from R-241
- Offline-doable now: seed/fixture data + handling the endpoints still left as 501 (sub-collections like
  /posts/{postId}/comments, custom/multi-param routes); flow IR roles into auth/route guards + a
  users/roles schema; a combined build/verify/preview plan surface tying runtime (R-233/234), verify
  (R-235), and edit (R-237) together for the console/CLI; or a richer diff (rename/hunk-level) on the
  R-237 ProjectDiff.
...
(R-241) with its task contract before writing code. Commit to main.
```
