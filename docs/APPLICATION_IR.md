...
- `Field.validation` (R-250/R-251) carries small deterministic rule strings the generators honour:
  `max_length:<int>` (schema `VARCHAR(n)` + Pydantic `Field(max_length=n)` + Go `max`), `enum:<a>|<b>|<c>`
  (schema `CHECK (col IN (...))` + Pydantic `Literal[...]` + Go `oneof`), and numeric `min:<n>` / `max:<n>`
  (schema `CHECK (col >= n)` / `CHECK (col <= n)` + Pydantic `ge`/`le` + Go `gte`/`lte`). Unknown rules
